...
export function buildNodeToCode(rawNodes: NodeMap) {
  ...
  for (const n of triggerNode) {
    ...
    if (n.type == "triggerOnMessageSend")
      result += `client.on("message",async function(${replaceSpecialChar(n.id)}_eventMessageSendData_result){\n${parseNode(n, rawNodes, variable)}})\n`
  }
...
function parseNode(node: FlumeNode, rawNodesList: NodeMap, variable: Record<string, string>) {
  ...
  switch (node.type) {
    ...
    case "compareNode": {
      ...
      result += `const ${replaceSpecialChar(node.id)}_result_result = ${data1} ${compareType == "equal" ? "==" : ""} ${data2};\n`
    ...
    case "sendMessage": {
      ...
      result += `const ${replaceSpecialChar(node.id)}_result_result = await ${messageVariableName}.channel.send(\`${content}\`);\n`
      variable[`${node.id}_result`] = `${replaceSpecialChar(node.id)}_result_result`
      break
    }
    default:
